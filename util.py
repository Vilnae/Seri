...
def parse():
    ...
    parser.add_argument(
        "-e", "--epub", action="store_true", help="Compile serial into epub"
    )
    parser.add_argument(
    ...
    parser.add_argument("-fn", "--frontnote", action="store_true", help="Add Author's notes")
    parser.add_argument("-en", "--endnote", action="store_true", help="Add Author's notes")
...
def in_list(type, tag):
    if type == "next":
        accepts = ["next chapter", "next", "next part", "next –>"]
        text = tag.text
        ...
        accepts = ["font-white", "entry-title", "leader", "chapter-title"]
        if not tag.has_attr("class"):
            return False
        if len(tag["class"]) > 1:
            text = " ".join(tag["class"])
        ...
        accepts = ["chapter-inner chapter-content", "entry-content", "hentry", "chapter-content", "chp_raw"]
        if not tag.has_attr("class"):
            return False
        if len(tag["class"]) > 1:
            text = " ".join(tag["class"])
